...
class GenerateAPITest(TestCase):
    ...
    def test_generate(self):
        generate_text_patch = unittest.mock.patch('letter.views.generate_text', return_value="Iloveyou")
        ...
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(force_str(response.content), {'generatedText': 'Iloveyou'})
...
class GenerateUnitTest(TestCase):
    def test_generate_text(self):
        ...
        requests_patch = unittest.mock.patch('letter.letter.requests.post', return_value=response_mock)
        with requests_patch:
            self.assertEqual(generate_text('letter-type', 'feel', 'voice'), 'Iloveyou')
